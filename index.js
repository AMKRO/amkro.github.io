...
const sigmers = [
   ...
   { name: "Cody", age: 17, residence: "Gisborne", hair: "Blonde"}
]
...
sigmers.forEach(sigmer => {
    const option = document.createElement("option")
    option.value  = sigmer.name;
    datalist.appendChild(option);
})

function checkGuess(guessName) {
    const guessedSigmer = sigmers.find(s => s.name.toLowerCase() == guessName.toLowerCase());
    if (!guessedSigmer) {
        alert("Sigmer not found!")
        return
    }
    const feedback = {};
    for (let key in trueSigmer) {
        if (typeof trueSigmer[key] == "number") {
            if (guessedSigmer[key] === trueSigmer[key]) feedback[key] = guessedSigmer[key] + " (✓)";
            else if (guessedSigmer[key] > trueSigmer[key]) feedback[key] = guessedSigmer[key] + " (down)"
            else feedback[key] = guessedSigmer[key] + " (up)"
        } else {
            feedback[key] = guessedSigmer[key] === trueSigmer[key] ? guessedSigmer[key] + " (✓)" : guessedSigmer[key] + " (X)";
        }
    }
    return feedback;
}
...
function isWin(feedback) {
    return Object.values(feedback).every(value => value.includes("✓"));
}
...
form.addEventListener('submit', function(e){
    ...
    if (totalGuesses >= guessLimit) {
        ...
        resetGame(); // <--- reset game on loss too
        return;
    }
...
toggle.addEventListener('change', function() {
  if (toggle.checked) {
    ...
    guessLimit = 3; // fewer guesses on hard
  } else {
    label.textContent = 'Easy';
    guessLimit = 5; // more guesses on easy
  }
